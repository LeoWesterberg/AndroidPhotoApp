...
class NewPhotoFragment : Fragment() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_IMAGE_CAPTURE && resultCode == RESULT_OK) {
            ...
            submit_button.setOnClickListener {
                viewModel.addPoint(Point(viewModel.lastLatitude!!,viewModel.lastLongitude!!,imgUri))
                viewModel.printPoints()
                getFragmentManager()?.popBackStackImmediate();
            }
    ...
    companion object{
        ...
        private const val REQUEST_IMAGE_PERMISSION = 1
        private const val REQUEST_IMAGE_CAPTURE = 2
        private const val REQUEST_POSITION_PERMISSION = 3
    }
    }